...
class SavingGoalServiceTest {
  ...
  void service_GetSavingGoal_ReturnSavingGoal() {
    ...
    when(savingGoalMapper.mapToSavingGoalDto(goal))
            .thenReturn(SavingGoalUtility.createSavingGoalDto());
    when(savingContributionRepository.findByUser_EmailAndGoal_Id(principal.getName(), 1L))
            .thenReturn(Optional.ofNullable(SavingGoalUtility.createSavingContributionA()));
  ...
  void service_GetAllGoalIdsByEmail_ReturnsListOfSavingGoalIds() {
    ...
    when(savingContributionRepository
            .findAllContributionsByUser_Email(
  ...
  void service_AddSavingGoal_AddSavingGoal() {
    ...
    when(savingGoalMapper.mapToSavingGoal(SavingGoalUtility
            .createSavingGoalCreationRequestDto(), user)).thenReturn(goal);
    when(savingGoalRepository.save(goal)).thenReturn(goal);

    // Act and assert
    assertDoesNotThrow(() -> savingGoalService.createSavingGoal(
            principal, SavingGoalUtility.createSavingGoalCreationRequestDto()));
    assertNotNull(savingGoalService.createSavingGoal(
            principal, SavingGoalUtility.createSavingGoalCreationRequestDto()));
  }
  ...
  void service_DeleteSavingGoal_DeleteSavingGoal() {
    ...
    // Act and assert
    assertDoesNotThrow(() -> savingGoalService
            .deleteSavingGoal(principal, SavingGoalUtility.createSavingGoalIdDto()));
  }
  ...
  void service_EditLives_EditLives() {
    ...
    // Arrange
    when(savingGoalRepository.findById(1L)).thenReturn(Optional.ofNullable(goal));
    SavingGoalUpdateValueDto savingGoalUpdateValueDto =
            SavingGoalUtility.createSavingGoalUpdateValueDto();
    when(savingContributionRepository.findByUser_EmailAndGoal_Id(
            principal.getName(), 1L)).thenReturn(
                    Optional.ofNullable(SavingGoalUtility.createSavingContributionA()));
  ...
  void service_RegisterSavingContribution_RegisterSavingContribution() {
    ...
    SavingGoalContributionDto savingGoalContributionDto =
            SavingGoalUtility.createGoalContributionDto();

    // Arrange
    when(savingContributionRepository
            .findByUser_EmailAndGoal_Id(anyString(), anyLong()))
            .thenReturn(Optional.ofNullable(SavingGoalUtility.createSavingContributionA()));
  ...
  @Test
  @DisplayName("JUnit test for getContributorsToGoal method")
  void service_getContributorsToGoal_ReturnContributors() {
    // Arrange
    List<SavingContribution> contributions = List.of(SavingGoalUtility.createSavingContributionA(),
            SavingGoalUtility.createSavingContributionC());
    when(savingContributionRepository
            .findAllContributionsByGoal_Id(1L)).thenReturn(contributions);
    when(savingContributionRepository
            .findByUser_EmailAndGoal_Id(anyString(), anyLong()))
            .thenReturn(Optional.ofNullable(SavingGoalUtility.createSavingContributionA()));

    // Act
    List<SavingGoalContributorDto> returnedContributors =
            savingGoalService.getContributorsToGoal(principal, 1L);

    // Assert
    assertThat(returnedContributors).hasSize(2);
  }

  @Test
  @DisplayName("JUnit test for addGoalToUser method")
  void service_addGoalToUser_ReturnContributors() {
    // Arrange
    when(userRepository.findUserByEmailIgnoreCase(
            principal.getName()))
            .thenReturn(Optional.of(SavingGoalUtility.createUserA()));
    when(savingGoalRepository.findByJoinCode(anyString()))
            .thenReturn(Optional.ofNullable(goal));
    when(savingContributionRepository.findByUser_EmailAndGoal_Id(principal.getName(), 1L))
            .thenReturn(Optional.ofNullable(SavingGoalUtility.createSavingContributionA()));
    when(savingGoalMapper.mapToSavingGoalDto(any(SavingGoal.class)))
            .thenReturn(SavingGoalUtility.createSavingGoalDto());
    // Act and assert
    SavingGoalDto savingGoalDto = savingGoalService.addGoalToUser(
            principal, SavingGoalUtility.createAddSharedGoalToUserDtoA());
  ...
  @Test
  @DisplayName("JUnit test for updateGoalState method")
  void service_updateGoal_ReturnContributors() {
    // Arrange
    when(savingGoalRepository.findById(1L))
            .thenReturn(Optional.ofNullable(goal));
    when(savingGoalMapper.mapToSavingGoalIdDto(goal))
            .thenReturn(SavingGoalUtility.createSavingGoalIdDto1());
    when(savingContributionRepository.findByUser_EmailAndGoal_Id(principal.getName(), 1L))
            .thenReturn(Optional.ofNullable(SavingGoalUtility.createSavingContributionA()));
    when(savingGoalRepository.save(goal)).thenReturn(goal);
    ...
    // Act and assert
    SavingGoalIdDto savingGoalIdDto = savingGoalService
            .updateGoalState(principal, SavingGoalUtility.createSavingGoalStateA());
